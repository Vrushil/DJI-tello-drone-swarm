...
class CommsManager:
    ...
    def init_tellos(self, sn_list, get_status=True, first_ip=1, last_ip=254):
        ...
        #x=input("Would you like to start the program?")
        #if x=='':
        ...
        #force_start=input("Press enter to skip search")
        num = len(sn_list)
        search_timeout=time.time()+5
        while len(self.tellos) < num:
            ...
            for ip in possible_addr:
                ...
                #print("In possible addrs loop")
                # awake=awake+10

            #for connected_drones in self.tellos:

            #   if time.time()>awake:

                  #      for ip in [tello.ip for tello in self.tellos]:
                   #         self.control_socket.sendto('command'.encode(), (ip, self.control_port))
                    #    print("sending command for awake")
                     #   awake=awake+15
           # print(tello.ip)
           # if time.time()>search_timeout+3:
            #    for tello in self.tellos:
             #       print(tello.ip)
              #      self.control_socket.sendto('command'.encode(), (tello.ip, self.control_port))
              #      self.queue_command('command', 'Read','All')
               #     print("Keeping drones awake")
            ...
            for _ in range(0, 10):
                ...
                if len(self.tellos) >= num:
                    break
            if time.time()>search_timeout+15 :

                print( "Starting with %d Tello(s)" %len(self.tellos))
                break


            #if input()=='':
             #   print("User issued forced start")
              #  break
                #manual_start= input("Do you wish to start manually")
                #if manual_start=='x' or 'X':
                 #   print( "Starting with %d Tello(s) manually" %len(self.tellos))
        ...
        for tello in self.tellos:
            ...
            #if len(self.tellos)<=num:
             #   if time.time()>=awake:
              #      tello_cmd_id.append((tello.num, tello.add_to_command_queue('command', 'Read', None)))
               #     print("In awake command")
                #    awake=awake+10
                #print(tello_cmd_id)
    ...
    def _get_tello(self, ip):
       ...
       # print("Tello IP not found")
        #raise RuntimeError('Tello not found!')

    def _send_command(self, tello, cmd_id, command, command_type, on_error, timeout=15):
        ...
        print('[Command %s]Sent cmd: %s to Drone %s' % (tello.ip, command,tello.num))
        ...
        while log_entry.response is None:
            ...
            if now - time_sent > timeout:
                ...
                print('[Command  %s]Failed to send: %s to Drone %s' % (tello.ip, command,tello.num))
    ...
    def _receive_thread(self):
        ...
        while not self.terminate_comms:
            try:
                ...
                print('[Response %s]Received: %s from Drone %s' % (ip, response,tello.num))
                ...
                if not self.terminate_comms:
                    ...
                    print('[Socket Error]Exception socket.error for receiving_thread : %s' % exc)
        ...
        while not self.terminate_comms:
            ...
            try:
               # print("In status thread")
                ...
                if not self.terminate_comms:
                    # Report socket errors, but only if we've not told it to terminate_comms.
                    print('[Socket Error]Exception socket.error for status thread: %s' % exc)
